...
async def _post_single_comment(gh: GitHubClient, repo: str, pr_number: int, body: str, event: str):
...
async def main() -> int:
    ...
    for idx, batch in enumerate(batches, start=1):
        ...
        if settings.severity_gate.lower() != "off" and local_decision == "request_changes":
        ...
        header = header_base.replace("## 🤖 GPT Code Review", f"## 🤖 GPT Code Review (batch {idx}/{total_batches})")
        body = header + summary_md + footer

        if not inline_mode:
            await _post_single_comment(gh, repo, int(pr_number), body, event)
            print(f"[batch {idx}/{total_batches}] Posted single comment (decision: {final_decision}).")
            continue
        ...
        if comments_payload:
            ...
            # Fall back to single comment if no mappable inline comments
            await _post_single_comment(gh, repo, int(pr_number), body, event)
            print(f"[batch {idx}/{total_batches}] No inline placements; posted single comment (decision: {final_decision}).")
